...
import static upgrader.upgrade.AbstractApplicationState.SPACE_SEP;
...
/**
 * The application state where addition takes place.
 * 
 * oldLibraryName.jar newSplittedLibraryName.jar -s
 * 
 * @author Zabee
 */

public class SplitLibraries
  ...
  public String splittedLibrary;
  protected TreeMap<Path, List<String>> occurences;

  public SplitLibraries(Upgrader up) {
    this.up = up;
  }
  ...
  public void process() {
    ...
    List<Path> matchingFiles =
        walkTreeAndRetrieveFiles(up.getReplacementLibrariesDir(), ".jar", splittedLibrary);
    ...
    if (NumberUtils.isNumber(userInput) && Integer.parseInt(userInput) >= 0
      ...
      LocateForAddition newState = new LocateForAddition(up);
